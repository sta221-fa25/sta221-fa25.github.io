...
(a) How do the least squares estimates $\hat{\beta_0}, \hat{\beta_1}$ change when we transform the predictor variable $\bx \rightarrow \bx^*$. $\bx^* = a \bx + b \bone$? In other words, compare $\hat{\beta_0}, \hat{\beta_1}$ above to $\hat{\beta_0}^*$ and $\hat{\beta_1}^*$ below.

$$
\by = \bone \beta_0^* + \left(a\bx +  b \bone \right) \beta_1^* + \be
$$
How does $cor(\bx, \by)$ compare to $cor(\bx^*, \by)$?

(b) How do the least squares estimates change when we transform $\by \rightarrow \by^*$ according to the linear transformation: $\by^* = c \by + d \bone$? How does $cor(\bx, \by)$ compare to $cor(\bx, \by^*)$?
