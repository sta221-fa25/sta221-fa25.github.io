...
(a) Create a visualization showing the how the prevalence of heart disease varies by sex.

(b) Create a visualization showing how prevalence varies by age categories, "under 45", "under 61 (but over 45)", "over 61". Hint: you can use `case_when` in R to make each category within a `mutate()`. See `?case_when` for an example.

## Exercise 8

*Fit the model in R*

(a) Fit a model to predict coronary heart disease using the predictors `age` and `sex`. Allow for an interaction effect between the variables. (b) Interpret the each variable's effect on the odds of heart disease.


```{r}
#| echo: false
#| eval: false
fit = glm(chd ~ ., data = heart, family = "binomial")
heart$pred_prob <- predict(fit, type = "response")
summary(fit)
```
...
Now fit a model using all the predictors in the data set (no interaction effects) and compare your model to that from exercise 8 using AUC of ROC. Which do you prefer? Why? 

## Exercise 10 

*Sensitivity and specificity*

Using the "better" model from exercise 9, predict the probability $\hat{p}_i$ that each individual in the data set has heart disease. Next you will choose a threshold $p^*$, such that if $\hat{p}_i \geq p^*$, you predict the individual has heart disease. Choose $p^*$ so that you have an empirical sensitivity of 0.95. Report your $p^*$ and create a table showing the sensitivity and specificity of your logistic classifier.
