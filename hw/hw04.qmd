...
(b). Write the update in terms of $\by^*$, where $\by^* = \bX \beta_{n} + D^{-1} (\by - \bp)$.
...
(a) Fit a model to predict coronary heart disease using the predictors  `age` (numeric) and `sex`. Allow for an interaction effect between the variables. (b) Interpret each variable's effect on the odds of heart disease.
...
(a) If you want $p^*$ such that you have an empirical sensitivity of at least 0.95, what is the maximum number of False Negatives you can afford for a data set of your size? 

(b) Report $p^*$ so that you have maximize your specificity while having a sensitivity of at least 0.95. 

(c) Use your $p^*$ to create a confusion matrix.
