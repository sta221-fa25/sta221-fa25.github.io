...
# eyedata_df <- read_csv("https://sta221-fa25.github.io/data/eyedata_df.csv")
eyedata_df = read_csv("../data/trim32.csv")
```
...
```{r}
#| echo: false
datatable(eyedata_df, rownames = FALSE, options = list(pageLength = 5),
...
In this data set, we have measurements from 120 rats, each with expression levels recorded for 200 genes. These 200 genes serve as our predictor variables. The outcome of interest is the expression level of a specific gene called TRIM32, which plays a role in retinal development. Thus, each observation corresponds to one rat, with TRIM32 expression as a continuous response variable and 200 other gene expression levels as predictors.
...
X = model.matrix(y ~ 0 + ., data = eyedata_df)
Xscale = sqrt(colMeans(X^2)) # create root mean squared
...
```{r}
#| echo: true
ridge_fit = MASS::lm.ridge(y ~ 0 + ., data = eyedata_df, lambda = 1)
...
# (different than computed above)

beta_scaled_lmridge[1:5]
betahat_ridge[1:5,]
```
...
ridge_fit = MASS::lm.ridge(y ~ 0 + ., data = eyedata_df, lambda = lambdas)
